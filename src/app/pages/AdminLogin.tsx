...
export function AdminLogin() {
  ...
  // ── Step 1 submit: validate credentials, get OTP sent ────────────────────
  async function handleCredentials(e: React.FormEvent) {
    ...
    try {
      ...
      // Credentials accepted — move to OTP step
      setStep(2);
    } finally { setLoading(false); }
  }

  // ── Step 2 submit: verify OTP, receive session cookie ────────────────────
  async function handleOTP(e: React.FormEvent) {
    ...
    try {
      ...
      if (!result.success) { setError(result.error ?? "OTP verification failed."); return; }
      // Cookie is set by the server — update React state with the user profile
      ...
      >
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10">
          ...
          <div className="flex items-center gap-2 mb-6">
            {[1, 2].map(s => (
              <div key={s} className={`flex-1 h-1.5 rounded-full transition-colors ${s <= step ? "bg-[#1A5F3D]" : "bg-gray-200"}`} />
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.form key="step1"
                ...
                onSubmit={handleCredentials} className="space-y-4" noValidate
              >
                <div>
            ...
            {step === 2 && (
              <motion.form key="step2"
                initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.2 }}
                onSubmit={handleOTP} className="space-y-4" noValidate
              >
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1.5">Check your admin email inbox for the 6-digit code.</p>
                </div>

                <button type="submit" disabled={loading || otp.length < 6}
                  className="w-full py-3 bg-gradient-to-r from-gray-800 to-gray-700 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-70">
