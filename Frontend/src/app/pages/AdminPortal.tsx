...
import {
  ...
  Shield, Eye, Search, X, CheckCircle, AlertCircle, Save,
} from "lucide-react";
      ...
      <div className="flex-1 overflow-x-hidden">
        <header className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400">Admin: {adminUser?.fullName}</span>
          </div>
        </header>
                    ...
                    {users.slice(0, 5).map(u => {
                      ...
                      return (
                        <tr key={u.id} className="border-b border-gray-800 last:border-0">
                          <td className="py-2.5 text-xs font-medium">{u.fullName}</td>
...
// ─── Users Tab ────────────────────────────────────────────────────────────────
function UsersTab({ users, profiles }: { users: any[]; profiles: ReturnType<typeof getAllFinancialProfiles> }) {
